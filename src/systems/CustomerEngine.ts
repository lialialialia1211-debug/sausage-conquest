...
function generateOrder(_day: number, personality?: string): CustomerOrder {
  ...
  // Intersect with gameState.unlockedSausages as the authoritative source of truth.
  const allCandidates: string[] = ['flying-fish-roe', 'cheese', 'big-taste', 'big-wrap-small', 'great-wall'];
  const unlocked = allCandidates.filter(id => gameState.unlockedSausages.includes(id));
  // Always guarantee at least one option to avoid empty list
  if (unlocked.length === 0) unlocked.push('flying-fish-roe');
